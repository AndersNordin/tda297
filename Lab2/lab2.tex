...
\subsection{Causal order}
  ...
  that breaks causal order, then a process $P$ has to recieve a message $m_1$
  after that send a message $m_2$ this causes $m_1$ and $m_2$ to be causally
  related, to break the causal order, message $m_2$ has to be percieved by all
  nodes before $m_1$, but this is a contradiction hence message $m_1$ has to have
  a sequence number when it is recieved by $P$ and $m_2$ will get a higher sequence
  number when it is sent to the sequencer, and the nodes deliver the messages according
...
\section{Conclusion}
  ...
  even before the message is sent, then the message has to be multicasted, making the message complexity
